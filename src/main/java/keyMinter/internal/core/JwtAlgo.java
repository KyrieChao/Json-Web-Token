...
import keyMinter.internal.core.support.Prep;
...
public interface JwtAlgo {
     ...
     * 使用指定的密钥ID创建JWT算法的方法
     *
     * @param algorithm 算法对象，用于指定JWT使用的算法类型
     * @param path      密钥文件路径，指向用于签名或验证的密钥文件
     * @param keyId     密钥标识符，用于唯一标识使用的密钥
     * @param force     是否强制使用指定密钥的标志，当设置为true时会忽略可能存在的错误
     * @return 返回一个配置了密钥ID的JwtAlgo对象，可用于JWT的生成和验证
     */
